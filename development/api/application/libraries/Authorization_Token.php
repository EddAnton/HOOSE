...
class Authorization_Token
{
	 ...
	 * ( 1 Hour ) : 60 * 60     = 3600
	 */
	// protected $token_expire_time = 604800; // 7 days
	// protected $token_expire_time = 86400;  // 1 day
	protected $token_expire_time = 28800; // 8 hrs
	// protected $token_expire_time = 14400; // 4 hrs
	/* Development */
	// protected $token_expire_time = 15; // 5 seconds
	...
	public function generateToken(&$data)
	{
		try {
			/*
			Modificado para incluir automáticamente:
				- Hora de generación del token
				- Hora de expiración
			*/
			$data['time'] = time();
			$data['expire'] = $data['time'] + $this->token_expire_time;
	...
	public function validateToken()
	{
		...
		if ($token_data['status'] === true) {
			try {
				 ...
				 */
				try {
					$token_decode = JWT::decode($headers[$token_data['key']], $this->token_key, [$this->token_algorithm]);
				} catch (Exception $e) {
