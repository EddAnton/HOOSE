...
abstract class REST_Controller extends CI_Controller
{
	...
	public function __construct($config = 'rest')
	{
		...
		if (
			$this->config->item('rest_database_group') &&
			($this->config->item('rest_enable_keys') || $this->config->item('rest_enable_logging'))
		) {
		...
		if (
			($this->auth_override === false && !($this->config->item('rest_enable_keys') && $this->_allow === true)) ||
	...
	public function _remap($object_called, $arguments = [])
	{
		...
		$object_called = preg_replace(
			'/^(.*)\.(?:' . implode('|', array_keys($this->_supported_formats)) . ')$/',
			'$1',
			$object_called
		);
		...
		$log_method = !(
			isset($this->methods[$controller_method]['log']) && $this->methods[$controller_method]['log'] === false
		);

		// Use keys for this method?
		$use_key = !(
			isset($this->methods[$controller_method]['key']) && $this->methods[$controller_method]['key'] === false
		);
		...
		if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === false) {
			...
			$this->response(
				[
					$this->config->item('rest_status_field_name') => false,
					// $this->config->item('rest_message_field_name') => sprintf($this->lang->line('text_rest_invalid_api_key_include_key'), $this->rest->key)
		...
		elseif ($data !== null) {
			...
			if (method_exists($this->format, 'to_' . $this->response->format)) {
				...
				$this->output->set_content_type(
					$this->_supported_formats[$this->response->format],
	...
	protected function _log_request($authorized = false)
	{
		...
		$is_inserted = $this->rest->db->insert($this->config->item('rest_logs_table'), [
			...
			'params' => $this->_args
				? ($this->config->item('rest_logs_json_params') === true
					? json_encode($this->_args)
					: serialize($this->_args))
				: null,
	...
	protected function _check_limit($controller_method)
	{
		...
		$time_limit = isset($this->methods[$limited_method_name]['time'])
			? $this->methods[$limited_method_name]['time']
			: 3600; // 3600 = 60 * 60
	...
	protected function _auth_override_check()
	{
		...
		if (!empty($auth_override_class_method_http)) {
			...
			if (
				!empty($auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method])
			) {
				// None auth override found, prepare nothing but send back a TRUE override flag
				if (
					$auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] ===
					'none'
				) {
					return true;
				}
	...
	protected function _check_login($username = null, $password = false)
	{
		...
		if (!$this->config->item('auth_source') && $rest_auth === 'digest') {
			...
			return md5(
				$username .
					':' .
					$this->config->item('rest_realm') .
					':' .
					(isset($valid_logins[$username]) ? $valid_logins[$username] : '')
			);
		}
