...
/**
 *
 * Controlador: Usuarios_Administradores
 *
 * Este controlador realiza las operaciones relacionadas con los Usuarios Administradores
 *
 * @package   CodeIgniter
...
class Usuarios_Administradores extends REST_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('Usuario_model');
		$this->load->model('Usuario_Administrador_model');
	}

	public function index()
	{
		$this->response(APP_NAME . ' API / Usuarios Administradores :: Controller');
	}
	...
	public function listar_get($soloActivos = false)
	{
		...
		try {
			...
			if (empty($idUsuario) || !intval($idUsuario) || intval($idUsuario) < 1) {
				$response['administradores'] = $this->Usuario_model->listar_perfil(
					PERFIL_USUARIO_ADMINISTRADOR,
					$idCondominio,
					$soloActivos
				);
			} else {
				$response['administrador'] = $this->Usuario_Administrador_model->listar($idUsuario, $idCondominio);
			}

			$response['msg'] =
				!empty($response['administrador']) || !empty($response['administradores'])
	...
	public function insertar_post()
	{
		...
		try {
			...
			$response = $this->Usuario_Administrador_model->insertar($data);
	...
	public function actualizar_post()
	{
		...
		try {
			...
			// Obtener las reglas de validación para la inserción
			...
			// Información validada con éxito. Procede a la inserción
			$response = $this->Usuario_Administrador_model->actualizar($data);
			if (!$response['error']) {
				$responseCode = REST_Controller::HTTP_OK;
			}
		} catch (Exception $e) {
...
/* End of file Usuarios_Administradores.php */
/* Location: ./application/controllers/Usuarios_Administradores.php */
