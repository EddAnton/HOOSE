...
class Usuario_Condomino_model extends CI_Model
{
	...
	*/
	public function listar($idUsuario = 0, $idCondominio = 0)
	{
		try {
            ...
            (CASE WHEN cc.estatus = 1 THEN cc.contrato_archivo ELSE NULL END) contrato,
            u.estatus,
            e.edificio,
            un.id_unidad,
            un.unidad,
            ...
            cc.estatus contrato_activo'
				...
				->where([
					...
					'u.fk_id_perfil_usuario' => PERFIL_USUARIO_CONDOMINO,
	...
	*/
	public function insertar($data)
	{
		...
		try {
			...
			// Obtener la información referente al unidad
			$dataUnidad = $data['unidad'];
			// Obtener identificador para el contrato
			$archivo_contrato = !empty($dataUnidad['archivo_contrato']) ? $dataUnidad['archivo_contrato'] : null;
			unset($data['unidad']);
			unset($dataUnidad['archivo_contrato']);
			unset($data['archivo_imagen']);
			unset($data['archivo_identificacion_reverso']);
			unset($data['archivo_identificacion_anverso']);
			$data['fk_id_perfil_usuario'] = PERFIL_USUARIO_CONDOMINO;
			...
			if (!validar_campos('condominos_contratos', $dataUnidad)) {
				$respuesta['msg'] = 'Error de integridad de la información con respecto a la base de datos (Contrato).';
				return $respuesta;
			}

			// Validar que la unidad se encuentre disponible para renta
			if (
				/* $this->db
					->join(
						'edificios e',
					...
					->join('condominos_contratos cc', 'cc.fk_id_unidad = u.id_unidad', 'left')
					->join('usuarios us', 'us.id_usuario = cc.fk_id_usuario AND u.estatus = 1', 'left')
					->where('us.id_usuario IS NULL')
					->get_where('unidades u', ['u.id_unidad' => $dataUnidad['fk_id_unidad'], 'u.estatus' => 1])
					->num_rows() != 1 */
				$this->db
					->join(
						'edificios e',
					...
					->where(['u.id_unidad =' => $dataUnidad['fk_id_unidad'], 'u.estatus' => 1])
					// ->or_group_start()
					// ->where('cc.fecha_fin < "' . $dataUnidad['fecha_inicio'] . '"')
					// ->group_end()
					->get('unidades u')
					->num_rows() != 1
			) {
				$respuesta['msg'] =
					'La unidad no pertenece al mismo condominio o no se encuentra disponible para asignar al condomino.';
				return $respuesta;
			}
      ...
      */
			if (!empty($archivo_contrato)) {
				...
				$se_cargaron_archivos = true;
				$dataUnidad['contrato_archivo'] = $cargar_archivo['archivo_servidor'];
			}
			...
			// Insertar registro en condominos_contratos
			$dataUnidad['fk_id_usuario'] = $nuevoID;
			// Si existe error al almacenar el registro, obtiene descripción del mismo y aborta el proceso
			if (!$this->db->insert('condominos_contratos', $dataUnidad)) {
				$respuesta['msg'] = 'Código: ' . $this->db->error()['code'] . ' - ' . $this->db->error()['message'];
				$this->db->trans_rollback();
				borrar_directorio($carpeta_temporal_cargar_archivos);
				return $respuesta;
			...
			$respuesta['condomino'] = $this->listar($nuevoID, $data['fk_id_condominio']);
	...
	*/
	public function actualizar($data)
	{
		...
		try {
			...
			// Obtener información del arrendamiento
			$dataUnidad = $data['unidad'];
			// Obtener identificador para el contrato
			// $idCondominoContrato = $dataUnidad['id_condomino_contrato'];
			$archivo_contrato = !empty($dataUnidad['archivo_contrato']) ? $dataUnidad['archivo_contrato'] : null;
			$borrar_contrato = $dataUnidad['borrar_contrato'];
			// Obtener la información referente al colaborador

			// Eliminar elementos no necesarios de la data
			unset($data['unidad']);
			// unset($dataUnidad['id_condomino_contrato']);
			unset($dataUnidad['archivo_contrato']);
			unset($dataUnidad['borrar_contrato']);
			unset($data['archivo_identificacion_reverso']);
            ...
            u.imagen_archivo,
            cc.contrato_archivo,
            cc.fk_id_unidad id_unidad_actual'
				)
				->join('condominos_contratos cc', 'cc.fk_id_usuario = u.id_usuario', 'left')
			...
			$contrato = $this->db
				->select(
					'contrato_archivo,
            fk_id_unidad id_unidad_actual,
            estatus'
				)
				->order_by('id_condomino_contrato DESC')
				->get_where('condominos_contratos', [
					'fk_id_usuario' => $idUsuario,
				])
				->row_array();
			$reactivandoCondomino = !empty($contrato) && $contrato['estatus'] == 0;

			// Validar que la unidad se encuentre disponible para renta
			if (
				($reactivandoCondomino || $contrato['id_unidad_actual'] != $dataUnidad['fk_id_unidad']) &&
				$this->db
				...
				/*
        $unidadDisponible = $this->db
					->join('edificios e', 'e.id_edificio = u.fk_id_edificio AND e.fk_id_condominio = ' . $idCondominio)
					->join('condominos_contratos cc', 'cc.fk_id_unidad = u.id_unidad')
					->join('usuarios us', 'us.id_usuario = cc.fk_id_usuario AND us.estatus = 0')
					->where(['u.id_unidad =' => $dataUnidad['fk_id_unidad'], 'u.estatus' => 1])
					->get('unidades u')
					->num_rows() != 1
          ->get_compiled_select('unidades u');
          print_r($unidadDisponible);
          exit;
          */
				$respuesta['msg'] =
					'La unidad no pertenece al mismo condominio o no se encuentra disponible para asignar al condomino.';
			...
			$contrato_archivo_existente = $reactivandoCondomino ? '' : $contrato['contrato_archivo'];
      ...
      */
			...
			$resultadoCondominosContratos = false;
			if ($reactivandoCondomino) {
				$dataUnidad['fk_id_usuario'] = $idUsuario;
				$resultadoCondominosContratos = $this->db->insert('condominos_contratos', $dataUnidad);
			} else {
				$resultadoCondominosContratos = $this->db->update('condominos_contratos', $dataUnidad, [
					'fk_id_usuario' => $idUsuario,
					'fk_id_unidad' => $contrato['id_unidad_actual'],
					'estatus' => 1,
				]);
			}

			// Actualizar datos en condominos_contratos
			// Si existe error al almacenar el registro, obtiene descripción del mismo y aborta el proceso
			if (!$resultadoCondominosContratos) {
				$respuesta['msg'] = 'Código: ' . $this->db->error()['code'] . ' - ' . $this->db->error()['message'];
				$this->db->trans_rollback();
				// Borrar archivos cargados
				if (!empty($data['identificacion_anverso_archivo'])) {
			...
			if ($se_cargaron_archivos) {
				if (
					!empty($imagen_archivo_existente) &&
					$borrar_imagen &&
					file_exists($carpeta_cargar_archivos . $imagen_archivo_existente)
				) {
					unlink($carpeta_cargar_archivos . $imagen_archivo_existente);
				}
				if (
					!empty($identificacion_anverso_archivo_existente) &&
					$borrar_identificacion_anverso &&
					file_exists($carpeta_cargar_archivos . $identificacion_anverso_archivo_existente)
				) {
					unlink($carpeta_cargar_archivos . $identificacion_anverso_archivo_existente);
				}
				if (
					!empty($identificacion_reverso_archivo_existente) &&
					$borrar_identificacion_reverso &&
					file_exists($carpeta_cargar_archivos . $identificacion_reverso_archivo_existente)
				) {
					unlink($carpeta_cargar_archivos . $identificacion_reverso_archivo_existente);
				}

				if (
					!empty($contrato_archivo_existente) &&
					$borrar_contrato &&
					file_exists($carpeta_cargar_archivos . $contrato_archivo_existente)
				) {
			...
			$respuesta['condomino'] = $this->listar($idUsuario, $idCondominio);
			$respuesta['error'] = false;
		} catch (Exception $e) {
			throw new Exception(extraerErrorDesdeJSON($e->getMessage()));
			die();
	...
	/*
    Alterna el estatus del contrato
      $data => Información del contrato a actualizar
	*/
	/* public function alternar_estatus($data)
	{
		...
		try {
			...
			$idCondominoContrato = $data['id_condomino_contrato'];

			// Verificar cuantos registros serán actualizados
			$contrato = $this->db->get_where('condominos_contratos', [
			...
			// Almacenar información en BD
			$respuesta['err'] = !$this->db->update('condominos_contratos', $data, [
				'id_condomino_contrato' => $idCondominoContrato,
			]);
			$respuesta['msg'] = $respuesta['err']
				? 'Código: ' . $this->db->error()['code'] . ' - ' . $this->db->error()['message']
				: (isset($estatus) ? 'Contrato ' . ($estatus == 0 ? 'des' : '') . 'habilitado' : 'Estatus modificado') .
					' con éxito.';
		} catch (Exception $e) {
    ...
    Eliminar logicamente un registro
      $data => Información a insertar
	*/
	public function finalizar_contrato($data)
	{
		$respuesta = [
			'error' => true,
			'msg' => null,
		...
		try {
			...
			// Validar que los campos existan en la tabla condominos_contratos
			if (!validar_campos('usuarios', $dataUsuario)) {
				$respuesta['msg'] = 'Error de integridad de la información con respecto a la base de datos (Contrato).';
				return $respuesta;
			}
			...
			// Verificar cuantos registros serán actualizados
			$usuario = $this->db
				->select('cc.fk_id_unidad, cc.fecha_inicio')
				// ->join('condominos_contratos cc', 'cc.fk_id_usuario = u.id_usuario AND cc.fecha_fin IS NULL')
			...
			if ($usuario->row()->fecha_inicio > $dataContrato['fecha_fin']) {
			...
			// Si existe error al almacenar el registro, obtiene descripción del mismo y aborta el proceso
			if (
				!$this->db->update('condominos_contratos', $dataContrato, [
					'fk_id_usuario' => $idUsuario,
					'fk_id_unidad' => $usuario->row()->fk_id_unidad,
				])
			) {
				$respuesta['msg'] = 'Código: ' . $this->db->error()['code'] . ' - ' . $this->db->error()['message'];
				$this->db->trans_rollback();
			...
			$respuesta['condomino'] = $this->listar($idUsuario, $idCondominio);
			$respuesta['msg'] = 'Contrato finalizado con éxito.';
			$respuesta['error'] = false;
		} catch (Exception $e) {
			throw new Exception(extraerErrorDesdeJSON($e->getMessage()));
			die();
...
/* End of file Usuario_Condomino_model.php */
/* Location: ./application/models/Usuario_Condomino_model.php */
