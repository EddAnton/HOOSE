...
export class UsuariosColaboradoresService {
	...
	ListarColaborador(idUsuario: number = 0) {
		const url = 'colaboradores/' + idUsuario;
		const headers = new HttpHeaders({
			'X-API-KEY': environment.appKey,
			Authorization: this.sesionUsuarioService.obtenerToken(),
		});
	...
	Guardar(data: ColaboradorModel) {
		let url = 'colaboradores/' + (data.id_usuario == 0 ? 'insertar' : 'actualizar/' + data.id_usuario);
	...
	/* Deshabilitar(idUsuario: number = 0) {
		const url = 'colaboradores/deshabilitar/' + idUsuario;
		const headers = new HttpHeaders({
			'X-API-KEY': environment.appKey,
			Authorization: this.sesionUsuarioService.obtenerToken(),
		});
